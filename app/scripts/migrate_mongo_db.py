#这段代码是一个可配置的 MongoDB 数据迁移脚本，用来把源数据库的 collection 文档复制到目标数据库，
# 并支持排除、增量、预演、限量和迁移报告。
import argparse
import json
import os
...
# _split_csv("users, orders, logs") => ["users", "orders", "logs"]
def _split_csv(value: Optional[str]) -> List[str]:
    if not value:
        return []
    return [item.strip() for item in value.split(",") if item.strip()]

# --since 2026-07-01T00:00:00Z => 2026-07-01T00:00:00+00:00
...
# 只迁移 created_at 或 updated_at 大于等于指定时间的数据。
...
# 先拿到源库所有 collection
# 如果传了 include，只保留 include 里的
# 如果传了 exclude，再排除 exclude 里的
...
# 只有「直接运行这个文件」时才执行（python migrate_mongo_db.py）。
#  被别人 import 时不会跑。
